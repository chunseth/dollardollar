...
const recommendationProjectFields = new Set(["stage", "status", "target_customer", "problem_statement", "solution_summary", "revenue_model", "pricing_hypothesis", "validation_stage", "founder_goal", "founder_constraints", "first_dollar_path"]);
...
function entityMutationAffectsRecommendation(entity, values = {}, record = {}) {
  if (entity === "assumptions" || entity === "evidence") return true;
  if (!["tasks", "experiments"].includes(entity)) return false;
  // Only issue-linked operational work can change the selected issue's active
  // work. Titles, descriptions, scheduling, and other display-only edits do
  // not create a replacement recommendation.
  const linked = values.assumption_id !== undefined ? values.assumption_id : record.assumption_id;
  return Boolean(linked) && ["status", "assumption_id", "experiment_id"].some(field => values[field] !== undefined);
}
function projectMutationAffectsRecommendation(values) { return Object.keys(values).some(field => recommendationProjectFields.has(field)); }
function appliedChangeSetAffectsRecommendation(changeSet) {
  return (changeSet.application_metadata?.applied_record_types || []).some(type => ["belief", "evidence", "task", "experiment"].includes(type));
}
...
async function api(request, response, url, generatePlan = createPlan, generateAssistant = callOpenAIModel, proposeCofounderChangeSet = defaultProposeChangeSet) {
  ...
  if (url.pathname === "/api/onboarding/confirm" && method === "POST") {
    ...
    // Onboarding is the one path that can create a project and its first
    // unresolved issues together, so initialize exactly one recommendation.
    const recommendation = selectedAssumptions.length ? await recalculateRecommendation(project.id, { purpose: "initial_recommendation" }) : null;
    return send(response, 201, { project, recommendation });
  }
  ...
  if (parts[3] === "change-sets") {
    ...
    if (parts.length === 6 && parts[5] === "apply" && method === "POST") { const change_set = await applyApprovedChangeSet(projectId, changeSetId, context); const recommendation = appliedChangeSetAffectsRecommendation(change_set) ? await recalculateRecommendation(projectId, { purpose: "approved_recommendation_state_change" }) : null; return send(response, 200, { change_set, recommendation }); }
  ...
  if (parts[3] === "recommendation" && parts[4] === "history" && parts.length === 5 && method === "GET") return send(response, 200, { recommendations: await recommendationHistory(projectId, { query }) });
