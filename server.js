...
async function api(request, response, url, generatePlan = createPlan, generateAssistant = placeholderAssistant, proposeCofounderChangeSet = defaultProposeChangeSet) {
  ...
  if (parts[3] === "change-sets") {
    ...
    if (parts.length === 6 && parts[5] === "apply" && method === "POST") { const change_set = await applyApprovedChangeSet(projectId, changeSetId, context); const recommendation = await recalculateRecommendation(projectId, { purpose: "approved_change_set" }); return send(response, 200, { change_set, recommendation }); }
