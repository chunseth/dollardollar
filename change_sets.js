...
async function audit(client, projectId, actorType, actor, event, entityType, entityId, payload) { await client.query("INSERT INTO event_log (project_id,actor_type,actor_id,event_type,entity_type,entity_id,summary,payload) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)", [projectId, actorType, actor, event, entityType, entityId, `Change set ${event}`, payload]); }
...
async function applyApprovedChangeSet(projectId, changeSetId, context = {}) {
  ...
  try {
    return await transaction(async client => { const set = await withSet(client, projectId, changeSetId); if (set.status === "applied") return { ...set, idempotent: true }; if (set.status !== "approved" || (set.expires_at && new Date(set.expires_at) <= new Date())) throw fail("Change set is not approved for application"); const items = (await client.query("SELECT * FROM change_set_items WHERE change_set_id=$1 ORDER BY sequence_number FOR UPDATE", [set.id])).rows.filter(item => item.review_status === "approved"); if (!items.length) throw fail("Change set has no approved items"); await client.query("UPDATE change_sets SET status='applying' WHERE id=$1", [set.id]); for (const item of items) { const result = await applyItem(client, projectId, item, actor); await client.query("UPDATE change_set_items SET review_status='applied',application_result_metadata=$2 WHERE id=$1 RETURNING *", [item.id, result]); await audit(client, projectId, "founder", actor, "applied", item.record_type, result.entity_id, { change_set_id: set.id, item_id: item.id, operation: item.operation, before: item.operation === "create" ? null : item.original_payload, after: item.current_payload }); }
